...
Set `autoStart: false` if you need to wait for some condition before opening the stream. You can also pre-construct an `AgentClient` with `createAgentClient` and pass it via `client` — useful for tests and for sharing one client across multiple hook instances.
